...
public class ButterflyEntity extends AnimalEntity implements Flutterer {
    ...
    public void growUp(int i, boolean bl) {
        ...
        for (int wingCount = 0; wingCount < random.nextInt(3); i++) {
            ItemScatterer.spawn(method_48926(), this.getX(), this.getY(), this.getZ(), new ItemStack(WWItems.BUTTERFLY_WING));
        }
    }
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        if (player.getStackInHand(hand).isOf(Items.SUGAR) && player.getStackInHand(hand).getCount() >= 16) {
            if (!this.method_48926().isClient) {
                method_48926().setBlockState(this.getBlockPos(), WWBlocks.BUTTERFLY_SPAWN.getDefaultState());
                if (!player.isCreative()) player.getStackInHand(hand).decrement(16);
            } else {
                ParticleUtil.spawnParticle(method_48926(), this.getBlockPos(), ParticleTypes.HEART, UniformIntProvider.create(1, 4));
            }
