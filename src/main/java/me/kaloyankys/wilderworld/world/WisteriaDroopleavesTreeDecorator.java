...
public class WisteriaDroopleavesTreeDecorator extends AttachedToLeavesTreeDecorator {
        ...
        for (BlockPos blockPos : Util.copyShuffled(generator.getLeavesPositions(), random)) {
            ...
            if (random.nextInt(2) == 0) {
                if (generator.isAir(blockPos2.down(1))) {
                    if (random.nextInt(2) == 0) {
                        generator.replace(blockPos2, this.blockProvider.getBlockState(random, blockPos2).with(VerticalConnectorBlock.TYPE, 3));
                        generator.replace(blockPos2.down(1), this.blockProvider.getBlockState(random, blockPos2).with(VerticalConnectorBlock.TYPE, 1));
                    } else if (generator.isAir(blockPos.down(3))) {
                        generator.replace(blockPos2, this.blockProviderBlossom.getBlockState(random, blockPos2));
                        generator.replace(blockPos2.down(1), this.blockProvider.getBlockState(random, blockPos2).with(VerticalConnectorBlock.TYPE, 2));
                        generator.replace(blockPos2.down(2), this.blockProvider.getBlockState(random, blockPos2).with(VerticalConnectorBlock.TYPE, 1));
                    }
                }
            } else {
                if (random.nextInt(2) == 0) {
    ...
    private static TreeDecoratorType<WisteriaDroopleavesTreeDecorator> register() {
        ...
        Registry.register(Registry.TREE_DECORATOR_TYPE, new Identifier("wilderworld", "wisteria_decorator"), WISTERIA);
        return WISTERIA;
    }
}