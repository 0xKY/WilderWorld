...
public class WisteriaFoliagePlacer extends RandomSpreadFoliagePlacer {
    ...
    protected static void placeBlossomBlock(BiConsumer<BlockPos, BlockState> biConsumer, BlockPos blockPos, int size) {
    ...
    protected static void placeFoliageBlock(TestableWorld testableWorld, BiConsumer<BlockPos, BlockState> biConsumer, Random random, TreeFeatureConfig treeFeatureConfig, BlockPos blockPos) {
        if (TreeFeature.canReplace(testableWorld, blockPos)) {
            BlockState blockState = treeFeatureConfig.foliageProvider.getBlockState(random, blockPos);
            ...
            if (random.nextInt(10) == 0) {
                if (testableWorld.testBlockState(blockPos.down(2), BlockPredicate.make(Blocks.AIR)) && testableWorld.testBlockState(blockPos.down(1), BlockPredicate.make(Blocks.AIR)) && testableWorld.testBlockState(blockPos.down(1), BlockPredicate.make(Blocks.AIR))) {
                    ...
                    placeBlossomBlock(biConsumer, blockPos.down(1), 1);
                }
            }
        }
    }
