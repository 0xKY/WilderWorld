...
public class ConnectorUtil {
    ...
    public static boolean placeConditions(BlockPos blockPos, WorldView worldView) {
        return (worldView.getBlockState(blockPos.up()).isOf(WWBlocks.DROOPBLOOM) && ConnectorUtil.findTop(blockPos, (World) worldView, ((w, pos, state) -> false)) != null) || worldView.getBlockState(blockPos.up()).isSolidBlock(worldView, blockPos) || worldView.getBlockState(blockPos.up()).isIn(BlockTags.LEAVES) || worldView.getBlockState(blockPos.up()).isOf(WWBlocks.WISTERIA.LEAVES)|| worldView.getBlockState(blockPos.up()).isOf(WWBlocks.WISTERIA_BLOSSOM_LEAVES);
    }
}
